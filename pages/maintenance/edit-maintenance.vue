	...
	export default {
		...
		methods: {
			...
			updateMaintenance() {
				...
				if (uni.getSystemInfoSync().platform !== 'h5') {
					// 更新数据库
					const sql = `UPDATE maintenance SET carId = ?, carNumber = ?, maintenanceDate = ?, mileage = ?, cost = ?, items = ? WHERE id = ?`;
					const params = [this.selectedCar.id, this.selectedCar.carNumber, this.maintenance.maintenanceDate, this.maintenance.mileage, this.maintenance.cost, this.maintenance.items, this.maintenance.id];
