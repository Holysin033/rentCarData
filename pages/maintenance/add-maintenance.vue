	...
	export default {
		...
		methods: {
			...
			saveMaintenance() {
				...
				if (uni.getSystemInfoSync().platform !== 'h5') {
					// 保存到数据库
					const sql = `INSERT INTO maintenance (carId, carNumber, maintenanceDate, mileage, cost, items, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)`;
					const params = [this.selectedCar.id, this.selectedCar.carNumber, this.maintenance.maintenanceDate, this.maintenance.mileage, this.maintenance.cost, this.maintenance.items, new Date().toISOString()];
